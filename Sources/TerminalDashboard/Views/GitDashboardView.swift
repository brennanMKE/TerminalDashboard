...
private enum GitDashboardError: Error {
    case message(String)
}

extension GitDashboardError: CustomStringConvertible {
...
/// Full-screen root view for `tuidash git`.
///
/// Wraps `GitView` and provides help overlay toggling via the command
/// field (type `h` + Return to show/dismiss).
...
// MARK: - GitDashboardContent

/// Wraps `GitView` and intercepts the `h` command to toggle the help overlay.
...
// MARK: - GitDashboardContentInner

private struct GitDashboardContentInner: View {
    ...
    private var commandInputRow: some View {
        HStack(spacing: 1) {
            ...
            TextField(placeholder: confirmingPush.wrappedValue ? "y/n" : "p/u/r/o/h") { cmd in
    ...
    private func handleCommand(_ cmd: String) {
        ...
        case "o":
            launchTool()
        default:
            operationResult.wrappedValue = "Unknown command '\(first)'. Use: p push  u pull  r rebase  o tool  h help"
        }
    }
    ...
    private func runOperation(args: [String]) {
        ...
        Task { @MainActor in
            ...
            case .failure(let err):
                operationResult.wrappedValue = "Error: \(err.description)"
            }
        }
    }
    ...
    private static func runGit(
        args: [String],
        repoPath: String
    ) async -> Result<String, GitDashboardError> {
